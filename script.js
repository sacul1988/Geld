document.addEventListener('DOMContentLoaded', () => {
    ...
    addExpenseBtn.addEventListener('click', () => {
        ...
        if (expense && expense > 0) {
            ...
            // Historie speichern (als AUSGABE markiert = negativer Wert für den Badge)
            const historyNote = title ? `${title}: -€${expense.toFixed(2)}` : `Ausgabe: -€${expense.toFixed(2)}`;
            saveToHistory(newAmount, historyNote, -expense);
    ...
    [budgetModal, expenseModal, historyModal].forEach(modal => {
    ...
    window.deleteHistoryEntry = (index) => {
        if (confirm('Diesen Eintrag wirklich löschen?')) {
            ...
            if (history.length > 0) {
                ...
                resetSavingsBtn.style.display = 'none';
            }
            ...
            return `
                <li class="history-item">
                    ...
                    <div style="display: flex; align-items: center;">
                        ...
                        <button class="history-delete-btn" onclick="deleteHistoryEntry(${index})">&times;</button>
                    </div>
                </li>
            `;
    ...
    function checkSavingsStatus() {
        ...
        if (startTimestamp && dailyRate) {
            ...
            // Spar-Reset Button anzeigen
            resetSavingsBtn.style.display = 'block';
