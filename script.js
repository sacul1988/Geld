document.addEventListener('DOMContentLoaded', () => {
    ...
    const showDaysBtn = document.getElementById('show-days-btn');
    ...
    addExpenseBtn.addEventListener('click', () => {
        ...
        if (expense && expense > 0) {
            ...
            // DIE ZWEI-ARTEN-BERECHNUNG:
            const remainingDaysTotal = updateDaysRemaining(); // Inklusive heute
            const daysFromTomorrow = remainingDaysTotal - 1;
            
            if (daysFromTomorrow > 0) {
                // Ab der ersten Ausgabe: Kontostand / restliche Tage OHNE heute
                // Das berechnet exakt, was dir ab morgen PRO TAG zusteht.
                const newDailyRate = newAmount / daysFromTomorrow;
                
                localStorage.setItem('currentDailyBudget', newDailyRate.toString());
                dailyBudgetEl.innerText = `€ ${newDailyRate.toFixed(2)}`;
            } else {
                // Letzter Tag des Monats
                dailyBudgetEl.innerText = `€ ${newAmount.toFixed(2)}`;
                localStorage.setItem('currentDailyBudget', newAmount.toString());
            }
    ...
    showDaysBtn.addEventListener('click', () => {
        moreModal.style.display = 'none'; // Hauptmenü schließen
        const remainingDays = updateDaysRemaining();
        alert(`Verbleibende Tage bis Monatsende (inklusive heute): ${remainingDays}`);
    });
    ...
    function checkSavingsStatus() {
        ...
        if (startTimestamp && initialDailyRate) {
            ...
            // Wenn heute der Starttag ist, rechnen wir bis heute (inclusive),
            // damit das Tagesbudget sofort im Spar-Budget erscheint.
            // Ab dem Folgetag rechnen wir immer bis inklusive heute, 
            // da das Tagesbudget für den aktuellen Tag ja bereits "verfügbar" ist.
